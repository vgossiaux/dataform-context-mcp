---
name: dataform-context-golden-init
description: Construit interactivement le golden set de lineage colonne (.dataform-context/golden_columns.json) du repo Dataform courant, avec validation humaine de chaque tracé proposé. À utiliser pour amorcer ou compléter l'oracle manuel de validate-golden.
---
...
l'inverse. Sauf précision contraire de l'utilisateur, construis 5 entrées.
...
4. **Validation humaine.** Pose la question directement dans le chat, avec des choix
   clairs (valider / corriger / remplacer la colonne), en montrant : la colonne, les
   edges proposés, l'expression SQL, le `complete` attendu, et le **chemin du fichier**
   pour que l'utilisateur puisse vérifier dans le code lui-même. Attends sa réponse
